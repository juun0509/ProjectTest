...
<body>
	...
	<div id=board>
		...
		<table id="table">
			...
			<c:forEach var="board" items="${list}">
				<tr>
					...
					<td>${board.requestBoardId}</td>
					<!-- 게시물 번호 -->
					<td id="titletd"><a
						href="<c:url value='/requestBoard/read${ph.sc.queryString}&requestBoardId=${board.requestBoardId}'/>">${board.title }</a>
					</td>
					<td>${board.nick}</td>
					<fmt:formatDate var="today" value="${now}" pattern="yyyy.MM.dd" />
					<fmt:formatDate var="reg_date" value="${board.createdDate}"
						pattern="yyyy.MM.dd" />
					<c:choose>
						<c:when test="${ today <= reg_date }">
							<td class="regdate"><fmt:formatDate
									value="${board.createdDate}" pattern="HH:mm" /></td>
						</c:when>
						<c:otherwise>
							<td class="regdate"><fmt:formatDate
		...
		<div id="dBtn">
			<button type="button"
				onclick='location.href="<c:url value='/requestBoard/write'/>"'>글쓰기</button>
		</div>
		...
		<div id="page">
			<c:if test="${ph.showPrev }">
				...
				<a href="<c:url value='/requestBoard/list${ph.sc.getQueryString(ph.beginPage-1) }'/>">&lt;</a>
			</c:if>
			<c:forEach var="i" begin="${ph.beginPage }" end="${ph.endPage }">
				<a class='${ph.sc.page== i ? "active": "" }'
					href="<c:url value='/requestBoard/list${ ph.sc.getQueryString(i)}'/>">${i }</a>
			</c:forEach>
			<c:if test="${ph. showNext }">
				<a
					href="<c:url value='/requestBoard/list${ph.sc.getQueryString(ph.endPage+1) }'/>">&gt;</a>
