...
<html>
<head>
<link href="<c:url value="/resources/css/searchPage.css"/>"
	rel='stylesheet' />
<meta charset="UTF-8">
	...
	<div id="con">
		<div id="container">
			<div id="logo" onclick='location.href="<c:url value='/'/>"'>
				<h1>
					<b id="yy">tonic</b><b id="jj">bank</b>
				</h1>
			</div>
			...
			<fieldset id="search">
				<form action="search" method="get"
					...
					<input id="searcht" name="keyword" type="text"
						value="${ph.sc.keyword}" placeholder="검색어를 입력하세요"
						autocomplete="off">
			...
			<div id="side">
				...
				<c:set var="loginoutLink"
					value="${empty sessionScope.userId? '/login/login' :'/login/logout' }" />
				<c:set var="loginout"
					value="${empty sessionScope.userId?'로그인':'로그아웃' }" />
				<input type="submit" class="side" id="my" name="tabs" value="마이페이지">
				<c:if test="${empty sessionScope.userId }">
					<input type="submit" class="side" id="h" name="tabs"
						onclick="location.href='<c:url value="/signUp/join"/>'"
						value="회원가입">
				</c:if>
				<input type="submit" class="side" id="h" name="tabs"
					onclick="location.href='<c:url value="${loginoutLink }"/>'"
					value="${loginout }" checked>
				<c:if test="${not empty sessionScope.userId }">
					<input type="submit" class="side" id="l" name="tabs"
			...
			<div id="result">
				...
				<div class="result" style="margin-top: 0px;">
					<h4>
						자유게시판 (총 <b>${ph.totalCnt }</b>건)<a
							href="<c:url value='/more'/>?keyword=${ph.sc.keyword}">더보기</a>
					</h4>
					<ul>
						<c:choose>
				...
				<div id="result">
					<div class="result" style="margin-top: 0px;">
						<h4>
							(총 <b>${proPh.totalCnt }</b>건)
							<a href="<c:url value='/morepro'/>?keyword=${ph.sc.keyword}"> 더보기</a>
						</h4>
				...
				<div id="resultReview">
					<div class="result">
						<h4>
							후기 (총 <b>${ reviewPh.totalCnt }</b>건)
							<a href="<c:url value='/more'/>?keyword=${ph.sc.keyword}&searchOption=review">더보기</a>
						</h4>
						<ul>
							<c:choose>
								...
								<c:otherwise>
									<c:forEach var="review" items="${ reviewList }" begin="0" end="5">
										<fmt:formatDate var="today" value="${now}" pattern="yyyy.MM.dd" />
										<fmt:formatDate var="reg_date" value="${review.createdDate}" pattern="yyyy.MM.dd" />
										<li onclick="location.href='<c:url value="/community/read${ph.sc.queryString}&communityId=${review.communityId}"/>'">
											<a class="resultList">
												<div>
													<span class="listTitle">${review.title }</span>
												</div>
												<div>
													<span class="listContent">${review.content }</span>
												</div>
												<div>
													<span class="listCategory">${review.nick }</span>
													<span class="listCategory">
														<c:choose>
															<c:when test="${ board.category == 'free' }">자유</c:when>
															<c:otherwise>후기</c:otherwise>
													...
													<c:choose>
														<c:when test="${ today <= reg_date }">
															<span> 
																<fmt:formatDate value="${review.createdDate}" pattern="HH:mm" />
															</span>
														</c:when>
														<c:otherwise>
															<span>
				...
				<footer>
					<div id="ft">
						...
						<p id="ad">(우)26464 충청북도 청주시 **동 **로 *** (TEL)043-1234-5678
							사업자 등록번호: 202-81-50707</p>
						<p>
							<a href="#" title="f"></a>
							<a href="#" title="i"></a>
							<a href="#" title="k"></a>
